...
class SyncEngine:
    ...
    def sync_movements_99(self, local_cur, remote_cur, fecha_inv):
        ...
        for row in tqdm(rows, desc="Movimientos"):
            ...
            local_cur.execute(f"DELETE FROM tblReporteMovimientos WHERE IdArticulo = {row['IdArticulo']} AND TipoMovimiento = 99")
            local_cur.execute(f"DELETE FROM tblReporteMovimientos WHERE IdArticulo = {row['IdArticulo']} AND FechaAct <= '{format_sql_date(row['FechaAct'])}'")
            ...
            local_cur.execute(sql1 + sql2)
            
            # Marcar como transmitido en remoto
            remote_cur.execute(f"UPDATE tblReporteProcesosInventariosDif{self.id_sucursal} SET Transmitir = 0 WHERE IdArticulo = {row['IdArticulo']} AND TipoMovimiento = 99")
            self.db.local_conn.commit()
    ...
    def sync_articles(self, local_cur, remote_cur, fecha_act):
        ...
        for row in tqdm(rows, desc="Artículos"):
            local_cur.execute(f"DELETE FROM tblArticulos WHERE IdArticulo = {row['IdArticulo']}")
            ...
            local_cur.execute(f"INSERT INTO tblArticulos({fields}) VALUES({values})")
            
            # Inventario inicial si no existe
            local_cur.execute(f"SELECT IdArticulo FROM tblArticulosInventarios WHERE IdArticulo = {row['IdArticulo']}")
            if not local_cur.fetchone() and row['IdArticulo'] > 0:
                local_cur.execute(f"INSERT INTO tblArticulosInventarios(IdArticulo, IdSucursal, ExiInicial, Entradas, Salidas, ExiFinal, FechaAct) VALUES({row['IdArticulo']},{self.id_sucursal}, 0, 0, 0, 0, GETDATE())")
            
            # Paquetes
            local_cur.execute(f"DELETE FROM tblDetallePaquetes WHERE IdArticulo = {row['IdArticulo']}")
            remote_cur.execute(f"SELECT * FROM tblDetallePaquetes WHERE IdArticulo = {row['IdArticulo']} ORDER BY IdArticulo")
            paquetes = remote_cur.fetchall()
            for p in paquetes:
                local_cur.execute(f"INSERT INTO tblDetallePaquetes(IdArticulo, IdArticuloHijo, Cantidad, Descuento) VALUES({p['IdArticulo']},{p['IdArticuloHijo']},{p['Cantidad']},{p['Descuento']})")
    ...
    def sync_partners(self, local_cur, remote_cur, fecha_act):
        ...
        rows = remote_cur.fetchall()
        for row in rows:
            local_cur.execute(f"DELETE FROM tblSocios WHERE IdSocio = {row['IdSocio']}")
            ...
            local_cur.execute(f"INSERT INTO tblSocios({fields}) VALUES({vals})")
        self.db.local_conn.commit()
    ...
    def sync_prices(self, local_cur, remote_cur, fecha_act):
        ...
        with open(puerto, 'a') as f:
            ...
            for row in rows:
                ...
                remote_cur.execute(sql_rep + sql_vals)
                ...
                # Local update
                local_cur.execute(f"UPDATE tblArticulos SET ConPrecio = 1, Precio1 = {row['Precio1']}, Precio2 = {row['Precio2']}, Precio3 = {row['Precio3']}, Precio4 = {row['Precio4']} WHERE IdArticulo = {row['IdArticulo']}")
    ...
    def sync_lookups(self, local_cur, remote_cur, fecha_act):
        ...
        for table in tables:
            ...
            remote_cur.execute(f"SELECT * FROM {table} WHERE FechaAct >= '{fecha_act}'")
            rows = remote_cur.fetchall()
            ...
            for row in rows:
                local_cur.execute(f"DELETE FROM {table} WHERE {id_col} = {row[id_col]}")
                cols = ", ".join(row.keys())
                placeholders = ", ".join([valida_nulo(v, isinstance(v, str)) for v in row.values()])
                local_cur.execute(f"INSERT INTO {table}({cols}) VALUES({placeholders})")
        self.db.local_conn.commit()
    ...
    def repair_transfers(self, remote_cur):
        ...
        for row in remote_cur.fetchall():
            ...
            if max_iter == row['Iteracion']:
                ...
                remote_cur.execute(sql_ins)
            
            remote_cur.execute(f"UPDATE tblTraspasos SET FechaAct = NOW(), Reparado = 1 WHERE IdTraspaso = {row['IdTraspaso']}")

    def push_sales(self, local_cur, remote_cur):
        logging.info("Enviando Ventas...")
        # Lógica de tblVentasFast
        local_cur.execute("INSERT INTO tblVentasFast SELECT * FROM tblVentas WHERE FechaVenta > GETDATE()-7 AND IdVenta NOT IN (SELECT IdVenta FROM tblVentasFast)")
        self.db.local_conn.commit()

        local_cur.execute(f"SELECT * FROM tblVentasFast WHERE Transmitido = 0 AND IdSucursal = {self.id_sucursal} ORDER BY IdVenta")
        for row in local_cur.fetchall():
            logging.info(f"Enviando Venta {row.FolioVenta}")
            remote_cur.execute(f"DELETE FROM tblDetalleVentas WHERE IdVenta = {row.IdVenta} AND IdSucursal = {self.id_sucursal}")
            ...
            remote_cur.execute(f"REPLACE INTO tblVentas({cols}) VALUES({values})")
            
            # Detalle Ventas
            local_cur.execute(f"SELECT * FROM tblDetalleVentas WHERE IdVenta = {row.IdVenta} AND IdSucursal = {self.id_sucursal}")
            for d in local_cur.fetchall():
                remote_cur.execute(f"INSERT INTO tblDetalleVentas(IdVenta, IdSucursal, IdArticulo, Cantidad, PrecioVenta, Descuento, Iva, IdRenglon, IdTipoPrecio, Precio, DescripcionVenta, IdVentaCredito, IdSucursalVentaCredito, Defecto) VALUES({d.IdVenta},{d.IdSucursal},{d.IdArticulo},{d.Cantidad},{d.PrecioVenta},{d.Descuento},{d.IVA},{d.IdRenglon},{d.IdTipoPrecio},{d.Precio},{valida_nulo(d.DescripcionVenta,True)},{valida_nulo(d.IdVentaCredito)},{valida_nulo(d.IdSucursalVentaCredito)},{valida_nulo(d.Defecto)})")
            
            local_cur.execute(f"UPDATE tblVentas SET Transmitido = 1 WHERE IdVenta = {row.IdVenta} AND IdSucursal = {self.id_sucursal}")
            local_cur.execute(f"UPDATE tblVentasFast SET Transmitido = 1 WHERE IdVenta = {row.IdVenta} AND IdSucursal = {self.id_sucursal}")
        self.db.local_conn.commit()

    def push_consignments(self, local_cur, remote_cur):
        logging.info("Enviando Consignaciones...")
        local_cur.execute(f"SELECT * FROM tblConsignaciones WHERE Transmitido = 0 AND IdSucursal = {self.id_sucursal} ORDER BY IdConsignacion")
        for row in local_cur.fetchall():
            remote_cur.execute(f"DELETE FROM tblDetalleConsignaciones WHERE IdConsignacion = {row.IdConsignacion} AND IdSucursal = {self.id_sucursal}")
            ...
            remote_cur.execute(f"REPLACE INTO tblConsignaciones({cols}) VALUES({vals})")
            
            local_cur.execute(f"SELECT * FROM tblDetalleConsignaciones WHERE IdConsignacion = {row.IdConsignacion}")
            for d in local_cur.fetchall():
                remote_cur.execute(f"INSERT INTO tblDetalleConsignaciones(IdConsignacion, IdArticulo, Precio, CantidadSalida, IdRenglon, CantidadEntrada, IdSucursal) VALUES({d.IdConsignacion},{d.IdArticulo},{d.Precio},{d.CantidadSalida},{d.IdRenglon},{d.CantidadEntrada},{self.id_sucursal})")
            
            local_cur.execute(f"UPDATE tblConsignaciones SET Transmitido = 1 WHERE IdConsignacion = {row.IdConsignacion} AND IdSucursal = {self.id_sucursal}")
        self.db.local_conn.commit()
    ...
    def push_replenishment_config(self, local_cur, remote_cur):
        ...
        local_cur.execute(sql)
        for row in local_cur.fetchall():
            remote_cur.execute(f"REPLACE INTO tblConfiguracionResurtido(IdArticulo, IdSucursal, DiasMin, DiasMax, DiasSurtido, PVM15, ExiMinRes) VALUES({row.IdArticulo}, {self.id_sucursal}, {valida_nulo(row.DiasMin)}, {valida_nulo(row.DiasMax)}, {valida_nulo(row.DiasSurtido)}, {valida_nulo(row.PVM15)}, {valida_nulo(row.ExiMinRes)})")
            local_cur.execute(f"UPDATE tblBufferConfiguracionResurtido SET DiasSurtidoAnt = DiasSurtido, DiasMinAnt = DiasMin, DiasMaxAnt = DiasMax, PVM15Ant = PVM15, ExiMinResAnt = ExiMinRes WHERE IdArticulo = {row.IdArticulo}")
        self.db.local_conn.commit()

    def push_returns(self, local_cur, remote_cur):
        logging.info("Enviando Devoluciones...")
        local_cur.execute("SELECT * FROM tblDevoluciones WHERE Transmitido = 0 ORDER BY IdDevolucion")
        for row in local_cur.fetchall():
            remote_cur.execute(f"DELETE FROM tblDetalleDevoluciones WHERE IdDevolucion = {row.IdDevolucion} AND IdSucursal = {self.id_sucursal}")
            ...
            remote_cur.execute(f"REPLACE INTO tblDevoluciones({cols}) VALUES({vals})")
            
            local_cur.execute(f"SELECT * FROM tblDetalleDevoluciones WHERE IdDevolucion = {row.IdDevolucion} AND IdSucursal = {self.id_sucursal}")
            for d in local_cur.fetchall():
                remote_cur.execute(f"INSERT INTO tblDetalleDevoluciones(IdDevolucion, IdSucursal, IdArticulo, Cantidad, PrecioVenta, IdRenglon, Dev) VALUES({d.IdDevolucion},{d.IdSucursal},{d.IdArticulo},{d.Cantidad},{d.PrecioVenta},{d.IdRenglon},{d.Dev})")
            
            local_cur.execute(f"UPDATE tblDevoluciones SET Transmitido = 1 WHERE IdDevolucion = {row.IdDevolucion} AND IdSucursal = {self.id_sucursal}")
        self.db.local_conn.commit()

    def push_financial_records(self, local_cur, remote_cur):
        # Aperturas
        logging.info("Enviando Aperturas...")
        local_cur.execute("SELECT * FROM tblAperturasCierres WHERE Transmitido = 0 ORDER BY IdApertura")
        for row in local_cur.fetchall():
            ...
            remote_cur.execute(f"REPLACE INTO tblAperturasCierres({cols}) VALUES({vals})")
            local_cur.execute(f"UPDATE tblAperturasCierres SET Transmitido = 1 WHERE IdApertura = {row.IdApertura}")
        
        # Retiros
        logging.info("Enviando Retiros...")
        local_cur.execute("SELECT * FROM tblRetiros WHERE Transmitido = 0 ORDER BY IdRetiro")
        for row in local_cur.fetchall():
            ...
            remote_cur.execute(f"REPLACE INTO tblRetiros({cols}) VALUES({vals})")
            local_cur.execute(f"UPDATE tblRetiros SET Transmitido = 1 WHERE IdRetiro = {row.IdRetiro}")
        
        # Anticipos
        logging.info("Enviando Anticipos...")
        local_cur.execute("SELECT * FROM tblAnticipos WHERE Transmitido = 0 ORDER BY IdAnticipo")
        for row in local_cur.fetchall():
            remote_cur.execute(f"REPLACE INTO tblAnticipos(IdAnticipo, IdSocio, Anticipo, IdVenta, IdSucursal, Gasto, IdDevolucion) VALUES({row.IdAnticipo},{row.IdSocio},{row.Anticipo},{row.IdVenta},{self.id_sucursal},{row.Gasto},{row.IdDevolucion})")
            local_cur.execute(f"UPDATE tblAnticipos SET Transmitido = 1 WHERE IdAnticipo = {row.IdAnticipo}")
    ...
    def sync_final_movements(self, local_cur, remote_cur):
        ...
        local_cur.execute(f"DELETE FROM tblReporteMovimientos WHERE FechaMovimiento < '{fecha_corte_str}' AND TipoMovimiento <> 99")
        ...
        remote_cur.execute(sql)
        for row in remote_cur.fetchall():
            local_cur.execute(f"DELETE FROM tblReporteMovimientos WHERE IdArticulo = {row['IdArticulo']} AND TipoMovimiento = {row['TipoMovimiento']} AND Iteracion = {row['Iteracion']} AND Folio = {row['Folio']}")
            sql1 = "INSERT INTO tblReporteMovimientos(IdArticulo, IdSucursal, IdComputadora, TipoMovimiento, Folio, FechaMovimiento, Concepto, Mov, IdUsuario, EfectoInventario, Iteracion, FolioStr, FechaAct, EnLocal, Status) "
        ...
        remote_cur.execute(f"REPLACE INTO tblSucursalesTransmisiones(IdSucursal, IdComputadora, Computadora, Version, FechaTransmision) VALUES({self.id_sucursal},{self.id_computadora},'{self.settings['Computadora']}','{self.settings['Version']}', NOW())")
        self.db.local_conn.commit()
